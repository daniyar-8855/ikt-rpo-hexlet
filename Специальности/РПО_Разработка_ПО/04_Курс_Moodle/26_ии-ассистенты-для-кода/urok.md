...
> **Памятка.** Формулируя запрос на генерацию кода, собери его по модели **5S** (занятие 27): роль («Ты Python-разработчик») → конкретика с примером вход→выход → простой язык → формат ответа (код + пояснение) → уточнение в диалоге.
