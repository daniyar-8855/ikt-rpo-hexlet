...
> **Памятка.** Запрос к ИИ о баге собери по модели **5S** (занятие 27): роль → конкретика с примером (текст traceback + фрагмент кода, без секретов) → простой язык → формат ответа (причина + исправление) → уточнение в диалоге.
