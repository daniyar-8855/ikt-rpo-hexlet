...
> **Памятка.** Запрос к ИИ на любом этапе проекта собирай по модели **5S** (занятие 27): роль → конкретика с примером (обезличенные данные, вход→выход) → простой язык → формат ответа → уточнение в диалоге.
