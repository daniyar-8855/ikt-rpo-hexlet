...
## Занятие 02. Устройство ПК, операционные системы (Windows/Linux) и файловая система
...
**Главный вывод раздела.** Когда выбираешь компьютер под разработку, не гонись за «самым мощным процессором». Гораздо важнее **достаточно RAM (16 ГБ) и обязательно SSD**. Типичная ошибка новичка — взять ноутбук с топовым CPU, но 8 ГБ RAM и медленным HDD, и потом удивляться, почему «мощная» машина тормозит. Это прямо разбирается в упражнении 1 к занятию: из трёх конфигураций (топовый CPU + 8 ГБ + HDD; средний CPU + 16 ГБ + SSD; средний CPU + 8 ГБ + SSD) для веб-разработки с контейнерами правильный выбор — средняя: RAM и SSD важнее, а HDD в первой конфигурации — главное узкое место.
...
- **Процессами** — какие программы сейчас выполняются и в каком порядке получают процессор. Когда у тебя одновременно открыты браузер, редактор и музыка — это ОС переключает процессор между ними так быстро, что кажется, будто всё работает разом.
- **Памятью** — кому из программ сколько RAM выделить и как не дать одной программе залезть в память другой. Это защита: из-за неё сбой одной программы не роняет всю систему.
...
- **Доступом** — кто имеет право читать, изменять, запускать тот или иной файл. Отсюда растёт безопасность: не всякая программа может тронуть системные файлы.
...
- **WSL (Windows Subsystem for Linux)** — подсистема, позволяющая запускать настоящий Linux прямо в Windows [7];
- **контейнеры** (например, Docker) — «коробки» с программой и всем её окружением, которые одинаково запускаются и у тебя, и на сервере;
- **виртуальные машины** — отдельный «компьютер внутри компьютера» с другой ОС.
...
Почему именно так?
- **Кириллица** в путях и именах ломает многие команды терминала, импорты и развёртывание на серверах, где локаль может быть другой.
- **Пробелы** ломают команды: терминал воспринимает пробел как разделитель аргументов, и `файл 1.py` он прочитает как два разных слова. Приходится оборачивать в кавычки, и легко ошибиться.
- **Большие буквы** создают тот самый баг с регистром между Windows и Linux.

Эта привычка кажется мелочью, но именно она избавляет от целого класса «загадочных» ошибок при работе в команде и на сервере. Заведи её прямо сейчас, в учебных проектах.
...
1. **Другая ОС** — у тебя Windows, на сервере Linux (разное поведение с регистром, разные разделители путей).
...
5. **Разные версии** языка или библиотек.

Хорошая новость: всем этим различиям есть единое лекарство — **сделать окружение одинаковым и писать переносимый код**. Используй относительные пути, единый стиль имён (латиница, нижний регистр), а для совпадения системы — WSL или контейнеры. Тогда фраза «у меня работает» начинает означать «работает и у всех остальных». Это и есть профессиональная зрелость в работе с окружением.
...
- **Ошибка:** считать, что Linux можно не знать, «я же на Windows». **Почему:** код в продакшене работает на Linux-серверах, и поведение там отличается. **Как правильно:** учись ориентироваться в Linux; ставь WSL/контейнеры, чтобы среда совпадала с сервером.
...
- **Ошибка:** судить о содержимом файла только по расширению. **Почему:** расширение можно сменить, оно не меняет сам файл. **Как правильно:** помни, что расширение — лишь подсказка типа, а не гарантия.
...
2. пути внутри проекта — относительные, не абсолютные;
...
- ОС управляет процессами, памятью, файлами и доступом; понимание этого помогает читать ошибки.
