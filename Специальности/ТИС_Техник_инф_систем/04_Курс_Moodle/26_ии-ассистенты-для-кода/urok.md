...
Но именно здесь начинается настоящая работа техника информационных систем. ИИ дал черновик, а отвечаешь за код ты. Если не понять и не проверить ответ, можно сдать функцию, которая падает при делении на ноль или утекает чужие данные. Этот урок — про то, как сделать ИИ-ассистента помощником, а не источником багов.
...
ИИ-ассистенты уже стали частью повседневной работы техника ИС: они пишут черновики функций, объясняют чужой код, подсказывают синтаксис, помогают разобрать ошибку. Это сильно ускоряет работу — но только если ты умеешь ставить задачу и проверять результат.

Связь с профессией: работодатель ценит не того, кто умеет «нажать кнопку и скопировать», а того, кто умеет **поставить ИИ точную задачу и проверить ответ**. Навык продуктивной и ответственной работы с ИИ-ассистентом сегодня — обязательная часть профессии техника ИС.
...
Тот же приём работает и со скриптами администрирования — второй половиной работы техника ИС. Например: «Напиши на Bash скрипт, который проверяет свободное место на диске и, если занято больше 90 %, выводит предупреждение; добавь короткие комментарии». Запрос точный (что проверить, порог, формат вывода) — и ИИ вернёт почти готовый черновик, но понять его построчно и запустить сначала на тестовой машине всё равно должен ты.

> **Памятка.** Формулируя запрос к ИИ на генерацию кода или скрипта, собери его по модели 5S (занятие 27): роль → конкретика с примером вход→выход → простой язык → формат ответа → уточнение в диалоге.
...
**Ошибка.** Принять сгенерированный фрагмент как готовый результат и вставить его в проект, не разобрав построчно, — «ИИ же написал».

**Почему это ошибка.** ИИ даёт только **черновик**, а понимание и ответственность за результат остаются на тебе. Пока ты не можешь объяснить каждую строку своими словами, ты не заметишь ни скрытого бага, ни дыры в безопасности (пароль в открытом виде, незащищённый запрос к базе, необработанный ввод) — и не починишь код, когда он сломается на защите.

**Как правильно.** Относись к ответу как к черновику, за который отвечаешь ты: разбери код **построчно** (можешь объяснить — значит понял), проверь базовую безопасность (нет секретов в коде, ввод обработан, запросы к данным защищены) и протестируй на крайних случаях — и только потом используй. И никогда не отправляй ИИ реальные пароли, ключи API и персональные данные: их нужно обезличивать.
...
- Почему за код, написанный с помощью ИИ, отвечает техник ИС, а не ассистент?

## Итог

- Используй ИИ для черновика, объяснения кода и разбора ошибок.
- Формулируй точный запрос: язык, задача, вход/выход, ограничения.
- ИИ даёт черновик — понимание и ответственность на тебе: разбери код построчно и проверь базовую безопасность, прежде чем использовать (понимание → запуск → крайние случаи → безопасность).
