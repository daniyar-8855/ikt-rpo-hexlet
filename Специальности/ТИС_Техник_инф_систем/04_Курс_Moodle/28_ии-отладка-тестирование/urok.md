...
Код почти никогда не работает с первого раза. Большую часть рабочего времени техник информационных систем не пишет новый код, а **ищет и исправляет ошибки** в уже написанном. Кто умеет быстро находить причину бага и закрывать его тестом — экономит часы и не выпускает в продакшн поломанную программу.

Связь с профессией: отладка и тестирование — это ежедневная работа техника ИС. ИИ ускоряет разбор ошибок и генерацию тестов, но ответственность за качество кода остаётся на человеке. Умение работать с ИИ как с помощником, а не как с «оракулом», которому слепо верят, — ключевой профессиональный навык.
...
> ИИ хорош в объяснении ошибок и идеях, но доверие к коду даёт не ответ ИИ, а прогон тестов: гоняй код на обычном, крайнем и некорректном вводе — устоял на всех трёх, значит, исправление настоящее.

> **Памятка.** Прося ИИ найти причину бага, собери запрос по модели 5S (занятие 27): роль → конкретика (текст ошибки и фрагмент кода без секретов, как пример) → простой язык → формат (причина + исправление) → уточнение в диалоге, если гипотеза не подтвердилась.
...
Тот же алгоритм работает не только с программами, но и со скриптами администрирования — второй половиной работы техника ИС. Скрипт развёртывания падает с сообщением `Permission denied` при копировании файла — строка ошибки в логе сразу показывает причину: не хватает прав на каталог. Порядок тот же: прочитать сообщение → проверить гипотезу (права доступа) → исправить и перезапустить, а не менять скрипт наугад.
...
- Доверие к коду даёт не ИИ, а прогон тестов: любое исправление проверяй на обычном, крайнем и некорректном вводе.
