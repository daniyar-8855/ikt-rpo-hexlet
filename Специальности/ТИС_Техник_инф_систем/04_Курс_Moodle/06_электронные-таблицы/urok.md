# Использовать электронные таблицы для расчётов и данных

## Практическая ситуация

Руководитель техподдержки присылает тебе выгрузку: 200 заявок пользователей за месяц и время их обработки в часах. Нужно к завтрашнему утру: среднее время обработки заявки, сколько заявок закрыто в срок (за ≤ 24 часа), и наглядный график для отчёта. Писать ради этого программу — долго и не нужно.

Ты открываешь таблицу, вводишь пару формул, протягиваешь их вниз, нажимаешь «вставить диаграмму» — и через пять минут отчёт для руководителя готов. Именно так электронная таблица экономит часы: она считает за тебя, как только ты правильно записал формулу и навёл порядок в данных.
...
Таблицы — это не только бухгалтерия. Для техника информационных систем это быстрый инструмент: свести реестр заявок пользователей, посчитать метрики мониторинга серверов и сети, провести инвентаризацию оборудования и лицензий, прикинуть бюджет на закупку оборудования — без написания кода. Это первый шаг к навыку работы с данными (data literacy), который нужен в любой IT-роли.

Связь с профессией: техник ИС постоянно сталкивается с данными — логами серверов, метриками мониторинга сети, реестрами заявок, учётом оборудования, ПО, лицензий, IP-адресов и учётных записей. Техник ИС тоже пишет скрипты, но, прежде чем разворачивать код для обработки, удобно быстро свести и проверить данные прямо в таблице — для оперативной проверки логов и метрик она часто быстрее кода. Кто умеет считать формулами и держать данные в порядке, тот тратит минуты там, где другие тратят часы.
...
Диапазон записывают через двоеточие: `A1:A10` значит «с A1 по A10». Пример с условием: `=ЕСЛИ(D2<=24; "в срок"; "просрочено")`.
...
Нужно из выгрузки 200 заявок с временем обработки посчитать среднее время и число заявок, закрытых в срок (≤ 24 ч). Решение: среднее время — `=СРЗНАЧ(B2:B201)`, число закрытых в срок — `=СЧЁТЕСЛИ(B2:B201; "<=24")`. Следующий шаг: выделить данные и построить столбчатую диаграмму по результатам — отчёт для руководителя готов.
...
2. Запиши формулы: (а) стоимость позиции = цена оборудования `B2` × количество `C2`; (б) среднее время обработки заявки по диапазону `B2:B201`; (в) число заявок, закрытых в срок (≤ 24 ч).

**Образец (часть ответа на пункт 2):** «Стоимость — `=B2*C2`; среднее время — `=СРЗНАЧ(B2:B201)`; число в срок — `=СЧЁТЕСЛИ(B2:B201; "<=24")`».
...
- Почему технику ИС бывает быстрее проверить данные (логи, метрики, реестр заявок) в таблице, чем сразу писать для этого код?
