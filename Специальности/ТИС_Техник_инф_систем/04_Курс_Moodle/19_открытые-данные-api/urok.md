...
Тебе нужно подготовить сводку: население городов Казахстана по годам — для отчёта, справочника или дашборда. Ты можешь «прикинуть» цифры на глаз — но тогда выводы будут недостоверны, и работу не примут всерьёз. А можно зайти на портал, где государство само выложило настоящую статистику, и взять проверенные числа.
...
Реальные данные — основа любой серьёзной работы техника ИС: отчётов о работе систем, сводок и справочников, мониторинга показателей — а также смежных задач: дашбордов, прототипов, учебного анализа, обучения моделей. Открытые данные дают их легально и бесплатно, потому что они обезличены и опубликованы для свободного использования.

Связь с профессией: техник информационных систем постоянно работает с данными — загружает наборы, подключается к чужим сервисам через API, разбирает ответы в формате JSON. Эти же данные нужны не только для разработки: из них собирают отчёты и дашборды, ими наполняют справочники и по ним ведут мониторинг работающих систем — сверяют показатели, автоматически подтягивают свежие сводки. Уметь находить открытые данные и понимать API — базовый навык и для разработки, и для сопровождения информационных систем.
...
Зачем технику ИС: реальные данные для отчётов и мониторинга систем, сбора сводок и проверки показателей, наполнения справочников — а также для смежных задач (дашборды, прототипы, учебные проекты, обучение моделей). Всё это без выдумывания и без нарушения приватности (данные обезличены и публичны).
