...
Те же четыре части работают и для скриптов администрирования — второй половины работы техника информационных систем. Промпт «Ты системный администратор. Напиши на Bash скрипт резервного копирования каталога `/var/www` в архив с датой в имени; учти пустой каталог; дай код и одну строку пояснения» задаёт роль, задачу, контекст и формат так же чётко, как промпт для функции на Python, — и черновик скрипта приходит почти готовым.
...
- Я понимаю: мусор на входе — мусор на выходе, поэтому «проверка» здесь означает уточнить и переформулировать сам промпт (роль, задача, контекст, формат), а не просто перезапросить.
...
- Почему в команде техников информационных систем умение точно ставить задачу ИИ похоже на умение ставить задачу младшему коллеге?
