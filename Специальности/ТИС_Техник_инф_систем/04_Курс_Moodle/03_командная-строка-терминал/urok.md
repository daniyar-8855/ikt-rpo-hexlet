# Освоить работу в командной строке/терминале
...
- объяснять, зачем технику информационных систем терминал, если есть мышка;
...
Терминал — это рабочий инструмент техника информационных систем на каждый день. Настроить сервер, установить и обновить ПО, развернуть проект и запустить систему, управлять службами и учётными записями, посмотреть логи, установить пакеты, закоммитить код, запустить тесты на сервере — всё это быстрее и надёжнее делается командами. Одна строка заменяет десяток кликов, а серия команд собирается в скрипт и повторяется автоматически.

Связь с профессией: инструменты техника информационных систем — и системные (управление службами, пакетами, пользователями, сетью, логами), и инструменты разработки (git, npm, pip, docker) — управляются из терминала. На сервере и удалённой машине графики часто просто нет — только терминал. Техник информационных систем, который боится командной строки, работает медленно и теряет контроль над системой. Уверенная работа в терминале — базовый профессиональный навык, без которого не обойтись.
...
- Я знаю, зачем технику информационных систем терминал, и могу назвать минимум два преимущества перед графикой.
...
- Почему привычка начинать с `pwd`/`ls` снижает риск дорогой ошибки в работе техника информационных систем?
