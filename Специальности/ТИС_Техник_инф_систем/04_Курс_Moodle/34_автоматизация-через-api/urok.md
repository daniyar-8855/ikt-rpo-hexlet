...
Связь с профессией: техник информационных систем работает с API с двух сторон. При разработке (ПМ 3) он обращается к чужим API (оплата, карты, уведомления, ИИ-сервисы) и проектирует API своего приложения. А в сопровождении (ПМ 7) через API автоматизирует эксплуатацию: сам собирает статус систем и показатели мониторинга, рассылает уведомления о сбоях, синхронизирует данные между сервисами — то, что раньше приходилось проверять и переписывать вручную. Это базовый навык — без него современную работу с информационными системами не представить.
...
- **сопровождение и мониторинг (ПМ 7)** — техник автоматизирует эксплуатацию: скрипт по API сам опрашивает сервисы, собирает их статус (`GET`) и, если система не отвечает, отправляет уведомление о сбое дежурному (`POST` в мессенджер).
