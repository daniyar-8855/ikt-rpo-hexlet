...
И вот почему это особенно важно именно для тебя как для будущего техника информационных систем. Обычный пользователь отвечает за свои данные — и точка. Техник ИС отвечает за гораздо большее: за код, за инфраструктуру, за чужие данные, за права доступа пользователей и за ключи к рабочим системам, которые он обслуживает. Утёкший в репозиторий ключ, незакрытая уязвимость на сервере, выданные «на всякий случай» лишние права или бэкап, который никто не проверял, — это уже не личная неприятность, а **инцидент**, за который отвечает вся служба поддержки, а иногда и компания перед законом. Поэтому цифровая гигиена для техника ИС — не «полезный совет со звёздочкой», а часть профессиональной культуры, наравне с умением писать чистый код и настраивать надёжную систему [1; 8]. И отвечает техник за цифровую гигиену не только свою, но и всей организации, которую обслуживает: пользователи будут работать так, как ты им настроил. Освоишь эти привычки сейчас, на учебных проектах, — придёшь в команду уже с правильными руками. Не освоишь — научишься после первого дорогого инцидента, а это куда обиднее.
...
- безопасно обращаться с персональными данными пользователей по требованиям закона РК и осознанно управлять своим цифровым следом;
- правильно хранить секреты в коде и на сервере (`.env`, `.gitignore`, переменные окружения) и понимать, почему ключ, попавший в историю git, считается скомпрометированным навсегда;
- действовать по порядку при инциденте и безопасно утилизировать оборудование и носители с данными;
- проводить базовый аудит собственной цифровой безопасности и цифровой гигиены обслуживаемых рабочих мест.
...
**Целостность (Integrity)** — данные не искажены и не подменены. Это свойство почти всегда забывают — а оно, поверь, критично, и провалы тут самые коварные, потому что незаметные. Представь: кто-то тихо изменил одну цифру в базе оценок, или в финансовом отчёте, или в коде программы. Данные на месте, никто их не «украл», сигнализация молчит — но они врут, и ты принимаешь решения по вранью. Для техника ИС целостность — это ещё и про код и про конфигурации: система контроля версий git хранит историю изменений именно для того, чтобы было видно, кто и что менял, и можно было откатить подмену; тот же смысл — у журналов (логов) на сервере, которые фиксируют, кто и когда менял настройки и данные. Инструменты — контроль версий, контрольные суммы, проверки и валидация данных.
...
Сюда же, к теме доступа, относятся ещё несколько привычек, про которые забывают:
- **раздавай права по принципу минимальных привилегий.** Это ключевая обязанность техника ИС, когда он заводит учётные записи и настраивает доступ пользователям. Правило простое: у каждого пользователя ровно те права, что нужны ему для работы, и ни одним больше. Не давай обычному пользователю прав администратора «чтобы не бегать по мелочи», не открывай доступ ко всей общей папке, если человеку нужен один подкаталог. Каждое лишнее право — это лишняя открытая дверь, через которую утекут данные, если этот аккаунт взломают или его владелец ошибётся. Отдельно следи за «осиротевшими» доступами: уволился сотрудник — сразу отзови его учётную запись и права.
- **разграничивай личное и рабочее.** Рабочие задачи — на рабочих аккаунтах и устройствах, личные — на личных. Не заходи в рабочие системы с личного непроверенного устройства, не пересылай рабочие файлы на личную почту «чтобы доделать дома». Смешение личного и рабочего — это дыра, через которую личный инцидент превращается в рабочий и наоборот.
...
Поэтому:
- **ставь обновления и патчи безопасности операционной системы и программ** — не только на своём ПК, но и на рабочих местах пользователей и на серверах, которые ты обслуживаешь; особенно следи за браузером, именно через него приходит большинство атак на обычного человека;
- по возможности включи **автоматические обновления**, чтобы защита не зависела от дисциплины и настроения (а настроение всегда против обновлений в неподходящий момент); там, где автообновление недопустимо (рабочий сервер), планируй установку патчей в окно обслуживания;
- обновляй и **зависимости в своих проектах** — библиотеки, которые использует твой код. Уязвимость в чужой библиотеке автоматически становится уязвимостью твоей программы, хотя ты сам ничего плохого не написал. Это уже сугубо профессиональная привычка техника ИС, и в командах за ней следят.
...
До сих пор мы защищали **твои** данные — это понятно и близко. Но как будущий техник информационных систем ты постоянно работаешь и с **чужими** данными — данными пользователей, которых обслуживаешь: они в базах, в заявках, в резервных копиях, в логах. И здесь к твоим привычкам добавляется ещё и закон, который спрашивает строго.
...
- **для разработки и тестирования используй тестовые или обезличенные данные**, а не реальные данные живых людей. На учебном проекте живые ИИН просто не нужны. То же правило действует и в работе техника ИС: когда выгружаешь данные для отладки, прикладываешь их к тикету или показываешь коллеге — сначала обезличь.
...
### 4.9. Связь с профессией: почему это ответственность всей организации

Сведём всё к главной профессиональной мысли, ради которой тема вообще стоит в программе техника информационных систем. Для обычного человека утечка пароля — личная неприятность, сам напортачил — сам разгребаешь. Для техника ИС утечка ключа, незакрытая уязвимость на сервере или слишком широкие права у пользователя — это **инцидент**, и слово это неслучайное: инцидент затрагивает не только его одного. Утёкший в репозиторий ключ открывает доступ к рабочим системам всей команды и всех пользователей, которых техник обслуживает. Реальные данные пользователей, по лени оставленные в тестах или в логе, — это уже нарушение закона, за которое отвечает компания, а не «стажёр, который скопировал файлик». И тут важнейшая мысль профиля: техник ИС отвечает за цифровую гигиену не только свою, но и всей организации — пользователи будут работать ровно так безопасно, как он им настроил парольную политику, права доступа, обновления и резервное копирование. Поэтому в зрелых командах цифровая гигиена не висит на доброй воле, а встроена в процессы: ревью кода ловит секреты до публикации, есть чёткие правила работы с данными, принцип минимальных привилегий при выдаче доступа, обязательная 2FA, регулярные проверяемые бэкапы, плановая установка патчей.

Ещё две обязанности техника ИС по этой теме, которые часто не проговаривают вслух, но спрашивают строго. Первая — **реагирование на инцидент**. Когда что-то случилось (утёк ключ, взломали аккаунт, пропали данные, поймали шифровальщика), паниковать поздно, нужно действовать по порядку: зафиксировать, что произошло, ограничить ущерб (сменить скомпрометированные пароли и ключи, отключить затронутый аккаунт или сервис от сети), восстановить работу из проверенной резервной копии, сообщить ответственным и разобрать причину, чтобы не повторилось. Именно ради этого шага заранее и нужны логи и бэкапы — без них инцидент нечем расследовать и не из чего восстанавливаться. Вторая — **безопасная утилизация оборудования и носителей**. Когда со списанного компьютера, диска или флешки просто «удаляют файлы» и отдают технику дальше, данные на них остаются и восстанавливаются обычными утилитами. Поэтому носители перед выводом из эксплуатации надёжно затирают (или физически уничтожают, если это диск с чувствительными данными), а не выбрасывают как есть. Списанное рабочее место с базой пользователей, ушедшее «на разбор» с читаемым диском, — это та же утечка, только отложенная.

Освоив привычки из этой темы сейчас, на учебных проектах, ты приходишь в команду уже с правильной культурой в руках — а не учишься всему этому в панике после первого дорогого инцидента, когда учиться уже поздно и стыдно.
...
**Пример 2 — резервные копии.** У пользователя, которого обслуживает техник ИС, все рабочие файлы только на одном ноутбуке.
...
**Пример 3 — утечка на скриншоте.** Техник ИС собирается приложить к отчёту скриншот, где видны: токен API интеграции, e-mail пользователей, путь `C:\Users\Ivan`.
*Разбор:* пройдёмся по каждому объекту, как на реальном ревью. Токен API — это секрет от рабочей системы, тут страдает конфиденциальность, закрыть обязательно. E-mail пользователей — это персональные данные, публикация без согласия нарушает приватность и закон, закрыть обязательно. Путь `C:\Users\Ivan` — не секрет, но он раскрывает имя пользователя системы, лишняя наводка для чужого, поэтому лучше обрезать. Правило из примера: перед публикацией любого скриншота специально проверять его на чувствительные данные, а не выкладывать на автомате.

**Пример 4 — секрет в коде.** Техник ИС записал ключ API прямо в код и закоммитил.
*Разбор:* в чём именно ошибка — ключ теперь навсегда останется в истории git, и его найдут боты за минуты, ещё до того как техник осознает промах. Правильный порядок: вынести ключ в `.env`, добавить `.env` в `.gitignore`, а уже скомпрометированный ключ немедленно отозвать и выпустить новый. И ключевое, где ошибаются даже те, кто про `.env` слышал: просто удалить строку из текущего файла — недостаточно, она остаётся в прошлых коммитах, а рабочий ключ уже гуляет.
...
- **Ошибка:** выдать пользователю прав больше, чем нужно, «на всякий случай». **Почему:** каждый лишний доступ — открытая дверь, через которую утекут данные при взломе или ошибке пользователя. **Как правильно:** принцип минимальных привилегий, а при увольнении сотрудника — сразу отзывать учётную запись и права.
- **Ошибка:** списать компьютер или флешку, просто «удалив файлы». **Почему:** удалённые файлы восстанавливаются обычными утилитами, данные пользователей уйдут вместе с железом. **Как правильно:** перед списанием надёжно затереть или физически уничтожить носитель с чувствительными данными.
- **Ошибка:** откладывать обновления. **Почему:** известная уязвимость — открытая дверь, про которую после выхода патча знают все. **Как правильно:** включи автообновления, особенно для браузера; на серверах ставь патчи в окно обслуживания.
...
- Права доступа пользователям — по принципу минимальных привилегий; личное и рабочее — раздельно.
- Обновления и патчи безопасности закрывают известные уязвимости на рабочих местах и серверах; откладывать их — держать открытой дверь, про которую знают все.
- Резервные копии по правилу 3-2-1 с проверкой восстановления спасают доступность данных.
- Чужие персональные данные пользователей защищены законом РК; для разработки, тестирования и передачи в поддержку бери тестовые/обезличенные данные.
- Секреты — в `.env` и `.gitignore` (на сервере — в переменных окружения); утёкший ключ скомпрометирован навсегда, его сразу меняют.
- Техник ИС отвечает за цифровую гигиену всей организации: умей реагировать на инцидент и безопасно утилизировать оборудование и носители.
...
7. Какие данные считаются персональными по закону РК и как безопасно использовать данные пользователей при разработке и в работе техника ИС?
8. Почему ключ, однажды попавший в репозиторий git, считается скомпрометированным, и что нужно сделать в первую очередь?
9. В чём состоит принцип минимальных привилегий и почему техник ИС не выдаёт пользователю лишних прав «на всякий случай»?
10. Что техник ИС делает по шагам при инциденте (утёк ключ, взломан аккаунт) и почему для этого заранее нужны логи и бэкапы?
11. Почему нельзя просто «удалить файлы» и отдать списанный диск дальше, и как утилизировать носители с чувствительными данными правильно?
...
- **Безопасная утилизация носителей** — надёжное затирание или физическое уничтожение диска/флешки перед списанием, чтобы данные с них нельзя было восстановить.
...
- **Инцидент (в ИБ)** — событие нарушения безопасности (утечка ключа, взлом аккаунта, потеря данных), требующее реагирования по порядку: зафиксировать, ограничить ущерб, восстановить, разобрать причину.
- **Информационная безопасность** — защита данных от утечки, искажения и потери.
- **Принцип минимальных привилегий** — правило выдавать пользователю только те права доступа, что нужны для работы, и ни одним больше.
