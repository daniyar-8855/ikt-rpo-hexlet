...
## СДЕЛАЙ САМ: настрой свою цифровую гигиену (по шагам)
...
- **Windows:** «Параметры → Учётные записи → Варианты входа» (задай вход по PIN/паролю) и «Параметры → Персонализация → Экран блокировки → Заставка» — поставь заставку на 5–10 минут с галочкой **«Начинать с экрана входа»**. Сочетание **Win+L** блокирует экран мгновенно.
- **macOS:** «Системные настройки → Экран блокировки» — запрашивать пароль после засыпания.
- **Телефон:** автоблокировка через 30–60 секунд + PIN/биометрия.
...
**Что должно получиться:** включённая 2FA, менеджер паролей с сохранёнными записями и одним новым сгенерированным паролём, работающая автоблокировка экрана, папка-копия на втором носителе с подтверждённым восстановлением.

**Что приложить:** скриншоты — (1) экран с включённой 2FA; (2) окно менеджера паролей со списком записей; (3) настройка автоблокировки; (4) копия папки на втором носителе/в облаке и открытый из копии файл. На скриншотах **скрой** сами пароли, секретные ключи/коды 2FA и персональные данные. Подпиши одним предложением, какое свойство защиты (конфиденциальность/целостность/доступность) закрывает каждый шаг.
