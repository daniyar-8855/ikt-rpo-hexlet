...
Связь с профессией: техник информационных систем постоянно работает с данными — из базы, из логов сервера, из журнала заявок, из выгрузки. Прежде чем строить отчёт или принимать решение, эти данные надо увидеть. Умение быстро построить честный график — рабочий инструмент, а не украшение.
...
Matplotlib — внешняя библиотека, её нет в Python по умолчанию: один раз её ставят в терминале командой `pip install matplotlib`, а строка `import matplotlib.pyplot as plt` подключает её в программу (модуль `pyplot` под коротким именем `plt`).
