...
Запись `df[df["cena"] >= 2000]` с двумя скобками на первый взгляд похожа на магию. На самом деле здесь два действия, вложенные друг в друга. Разберём их по шагам на нашей маленькой таблице (три товара с ценами 1500, 2000, 9000).

**Шаг 1. Внутреннее `df["cena"] >= 2000` — строим маску.** Мы берём весь столбец цен и сравниваем каждое значение с 2000. Pandas проверяет условие для каждой строки и возвращает не одно «да/нет», а целый столбец из `True` и `False` — по одному значению на строку. Его называют **маской**:

```python
print(df["cena"] >= 2000)
# 0    False   ← 1500 не >= 2000
# 1     True   ← 2000 >= 2000
# 2     True   ← 9000 >= 2000
```

**Шаг 2. Внешнее `df[ ... ]` — фильтруем по маске.** Теперь мы подставляем эту маску внутрь квадратных скобок самого `df`. Pandas проходит по строкам и оставляет только те, напротив которых в маске стоит `True`, а строки с `False` выбрасывает. Остаются товары «Клав.» (2000) и «Монитор» (9000), а «Мышь» (1500) отсеивается.
