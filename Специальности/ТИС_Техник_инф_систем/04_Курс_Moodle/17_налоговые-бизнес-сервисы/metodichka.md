...
> Это расширенный разбор темы для самостоятельного изучения — подробнее, чем конспект урока: каждый сервис, шаг и обязательство разобраны глубже, с примерами и пошаговыми алгоритмами. Читай его как «учебник по теме». Держи в голове главное: тема кажется «не про код», но именно она превращает тебя из «студента, который иногда что-то верстает за наличные» в исполнителя, с которым подписывают договор на серьёзные деньги.
...
- **регистрация полностью онлайн** — всё подписывается ЭЦП из дома, без очередей;
- **минимальная отчётность** на специальных режимах — штатный бухгалтер не нужен, многое автоматизировано в кабинете;
- **низкие налоги** по упрощённому режиму — просто процент с дохода, а не расчёты «доходы минус расходы»;
- **гибкость** — можно приостановить деятельность, а потом закрыть ИП — тоже онлайн.
...
И самое важное предупреждение всей темы: **ЭЦП — это твоя личность в цифровом мире.** Не «пароль от одного сайта», а личность целиком. Тот, у кого есть твой ключ и пароль к нему, может действовать от твоего имени: регистрировать на тебя ИП и целые компании, подписывать договоры, брать обязательства и кредиты. Поэтому ключ ЭЦП и пароль к нему нельзя передавать никому — ни «помощнику по регистрации», ни «специалисту, который всё оформит за 5 минут», ни приятелю, который «просто посмотрит». Подробнее — в разделе 4.7 и в кейсах, но заруби на носу уже здесь: свой ключ ты не отдаёшь никогда.
...
- **действующую ЭЦП** на физлицо (нет или срок вышел — сначала получи/перевыпусти, см. занятие 15; на просроченном ключе ничего не подпишешь);
- **установленный и запущенный NCALayer** (именно запущенный — иначе портал не увидит ключи, и кнопка «подписать» ничего не сделает);
- **свой ИИН** (подтянется автоматически, но держи под рукой);
- решение по двум вопросам заранее: **какой вид деятельности** заявляешь (для техника ИС это обычно деятельность в области ИТ, разработка и сопровождение ПО, консультирование) и **какой налоговый режим** выбираешь (об этом — раздел 4.6, прочитай его до регистрации).
...
- **вид(ы) деятельности** — из классификатора (для техника ИС — позиции, связанные с ИТ, разработкой и сопровождением ПО; можно указать несколько);
- **налоговый режим** — например, специальный режим на основе упрощённой декларации (почему его чаще берут фрилансеры — в разделе 4.6);
...
Запомни общий каркас — он пригодится и для задания, и в жизни: **вход с ЭЦП → заполнение заявления (вид деятельности, режим) → подпись ЭЦП → получение подтверждения.** По этой же логике устроено большинство бизнес-услуг на госпорталах РК.
...
Теперь отдельно про ИТ — здесь студенты чаще всего путаются. В Казахстане действуют **льготные налоговые режимы для участников технопарка «Astana Hub»** — это отдельный механизм для ИТ-компаний, а не «скидка для всех айтишников». Ключевой момент: **льготы Astana Hub рассчитаны на компании на общеустановленном (общем) режиме и НЕ сочетаются с упрощёнкой.** Это две разные дороги:
- **упрощённый режим** (упрощённая декларация, форма 910.00) — про малый бизнес **вообще**, в том числе про тебя как ИП-фрилансера: маленький процент с оборота, простая отчётность;
- **льготы Astana Hub** — отдельный механизм для ИТ-компаний на **общем** режиме, вступивших в технопарк; упрощёнку с этими льготами не совмещают.

Нельзя «быть на упрощёнке и заодно получать преференции Astana Hub» — это взаимоисключающие вещи. Начинающему фрилансеру почти всегда ближе упрощёнка; к модели Astana Hub присматриваются, когда вырастаешь в ИТ-компанию с приличными оборотами и командой. **Актуальные правила всегда проверяй на официальных ресурсах** (kgd.gov.kz, astanahub.com), а не по пересказам в чатах.
...
- **общеустановленный (общий) режим** — налог считается от прибыли (доходы минус расходы), отчётность сложнее и объёмнее. Оправдан, когда расходов много и их выгодно учитывать (закупки, аренда, зарплаты). Именно на этом режиме работают ИТ-компании, претендующие на льготы Astana Hub;
- **специальные налоговые режимы для малого бизнеса** — упрощённый расчёт. Самый популярный у фрилансеров — **режим на основе упрощённой декларации**: налог считается как процент от дохода (оборота), а отчётность — одна декларация (форма 910.00) раз в полугодие.
...
> Важно: конкретные цифры (проценты ставок, пороги дохода, размеры взносов) в этом материале намеренно не приводятся как «вечные» — законодатель их периодически меняет. Правильный профессиональный навык — не зазубрить число, а знать, **где** посмотреть актуальное: Налоговый кодекс на adilet.zan.kz и разъяснения на kgd.gov.kz. Та же логика, что с командами в терминале: не помнить всё наизусть, а уметь быстро найти достоверный источник.

**Базовые обязанности ИП** (общая логика, без привязки к меняющимся цифрам):
- **сдавать отчётность вовремя** — для упрощённой декларации это форма 910.00 раз в полугодие, строго в срок;
- **платить налог с дохода** по своему режиму и **обязательные взносы за себя** — социальные и пенсионные отчисления, взносы на медицинское страхование (как самозанятый ты платишь их сам за себя, за тебя это не делает работодатель);
- **вести учёт доходов** — фиксировать поступления, чтобы корректно заполнить декларацию;
- **выдавать документы клиентам** — договоры, счета, акты; при приёме платежей может требоваться чек (через онлайн-ККМ или мобильное приложение — в зависимости от режима);
...
**Ловушка 1 — «оформим ИП за вас, пришлите ключ ЭЦП и пароль».** Самая опасная схема. В объявлении предлагают «быстро и без хлопот» зарегистрировать ИП, а для этого просят прислать файл ключа ЭЦП и пароль к нему. Как только ты их передал — ты отдал свою цифровую личность в чужие руки: на тебя могут оформить не только ИП, но и фиктивные компании, кредиты, долги, подписать любые договоры. Распутывать это придётся долго и с юристами. **Правило железное: ключ и пароль ЭЦП не передаются никому и никогда.** Регистрируйся только сам. «Помощник», которому нужен твой ключ, — не помощник.

**Ловушка 2 — фишинговые сайты-двойники.** Мошенники поднимают сайты, похожие на egov.kz или salyk.kz — похожий адрес (с лишней буквой или другой зоной), тот же дизайн, — чтобы ты ввёл там данные или подписал «заявление». Защита: набирай адрес портала руками или заходи из официального проверенного источника; проверяй, что соединение защищённое и адрес точный до буквы; никогда не переходи на «госуслуги» по ссылкам из писем и мессенджеров.

**Ловушка 3 — «налоговая» звонит и требует срочно оплатить или сообщить данные.** Настоящие уведомления от налоговой приходят в кабинет налогоплательщика, а не голосом по телефону с давлением «срочно, иначе штраф». Если кто-то по телефону, представляясь налоговой, торопит, пугает и просит коды из СМС, пароли или оплату «прямо сейчас на этот счёт» — это мошенничество. Положи трубку и проверь информацию сам, зайдя в cabinet.salyk.kz. Государство не требует переводов на «карту сотрудника».

**Ловушка 4 — «помощники», регистрирующие ИП на тебя.** Бывает и наоборот: человека просят «помочь открыть ИП за вознаграждение», а потом используют этот ИП для обналичивания или ухода от налогов. Деньги достаются организаторам, а отвечать перед государством будешь ты — как оформленный предприниматель. Никогда не регистрируй ИП «для кого-то» и не отдавай управление своим ИП посторонним.
...
- **Ошибка:** думать, что «оплата на карту» удобна навсегда. **Почему:** корпоративные клиенты так платить не могут, нет правовой защиты, растёт риск перед государством. **Как правильно:** при росте заказов оформить ИП и работать по договорам.
...
- **Ошибка:** путать упрощёнку с льготами Astana Hub и считать, что они действуют одновременно. **Почему:** это две разные несовместимые вещи — упрощёнка для малого бизнеса, льготы Astana Hub для ИТ-компаний на общем режиме. **Как правильно:** выбирать режим осознанно и сверять условия на kgd.gov.kz и astanahub.com.
...
3. **Заполни заявление:** вид деятельности, налоговый режим, контактные данные — вдумчиво.
