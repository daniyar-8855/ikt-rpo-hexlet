...
Связь с профессией: технику информационных систем no-code помогает **быстро собрать прототип** и сэкономить время на типовых задачах. Ты не заменяешь им программирование, а используешь как инструмент: простое и срочное — на no-code, сложное и нагруженное — кодом. Хороший профильный пример для сопровождения (ПМ 7): за час без кода можно собрать **бота приёма заявок пользователей** — он принимает обращение, фиксирует его в таблице и уведомляет дежурного, снимая с техника ручную регистрацию.
...
В работе техника ИС такой бот — это **приём заявок пользователей** (ПМ 7): пользователь пишет боту о проблеме, бот по правилам уточняет категорию (сеть, доступ, оборудование, ПО), фиксирует заявку в таблице и уведомляет дежурного. Ручная регистрация обращений уходит, а техник занимается самим решением.
...
**Шаг 1. Создай бота у BotFather.** В поиске Telegram набери `@BotFather`, открой чат, нажми «Запустить» (Start) и отправь команду `/newbot`. BotFather попросит имя бота (любое, например «Справочник группы ТИС») и username — он должен заканчиваться на `bot` (например `tis_help_2026_bot`). После этого BotFather пришлёт сообщение с **токеном** — длинной строкой вида `1234567890:AA...`. *(Здесь уместен поясняющий скриншот ответа BotFather — но токен на скриншоте закрась/замажь: это пароль от бота, его нельзя показывать.)*
...
**Шаг 6 (по желанию — живые ответы).** Чтобы бот реально отвечал текстом на команды, подключи бесплатный визуальный конструктор ботов (бери актуальный на момент занятия визуальный конструктор ботов — их набор часто меняется; выбирай с бесплатным тарифом и не вводи данные банковской карты) — там мышкой задаёшь: «на команду `/raspisanie` ответить таким-то текстом». Это тот самый no-code: правило «сообщение → ответ» собирается кликами. Надёжнее конструкторов — короткий скрипт на Python (5–7 строк с библиотекой для Telegram): это ближе к твоим профильным модулям (ПМ 3).
