...
Связь с профессией: техник информационных систем постоянно смотрит на данные — логи серверов, метрики загрузки, журнал заявок техподдержки, результаты тестов, поведение пользователей. Умение быстро вытащить из них смысл с помощью ИИ и при этом не доверять слепо — рабочий навык. А обезличивание данных перед отправкой во внешний сервис — это ещё и требование закона, нарушение которого дорого обходится компании.
...
> **Памятка.** Формулируя запрос на анализ данных, собери его по модели 5S (занятие 27): роль → конкретика (обезличенные данные + контекст, с примером нужного вывода) → простой язык → формат (например, «3 вывода списком») → уточнение в диалоге по результату.

## Вывод ИИ — это гипотеза

У анализа данных здесь своя ловушка. ИИ видит только числа, которые ты дал, и, объясняя их, легко **выдумывает причину** («продажи упали из-за погоды») — это называют галлюцинацией — или **подменяет корреляцию причинностью**: заметив, что два показателя меняются вместе, заявляет, будто один вызывает другой, хотя это может быть простым совпадением. Реальных внешних причин ИИ не знает — он достраивает их по тому, что «обычно пишут».

Поэтому вывод про данные проверяют не «вообще», а **сверкой с самими числами**: пересчитать показатель, вернуться к исходной таблице, поискать третий фактор, который объясняет оба ряда сразу.

![Вывод ИИ как гипотеза, которую проверяет человек](assets/32_shema-vyvod-gipoteza-proverka.svg)

Схема показывает этот путь: вывод ИИ → гипотеза → проверка → решение. Ответственность за итог всегда остаётся на человеке, а не на ИИ.
...
**Как правильно.** Считать вывод ИИ гипотезой и проверять её сверкой с самими числами и фактами. Перед отправкой — **обезличивать** данные: убирать имена, ID, контакты, оставляя только показатели.
